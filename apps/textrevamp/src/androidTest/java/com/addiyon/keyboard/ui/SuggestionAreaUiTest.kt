...
class SuggestionAreaUiTest {
    ...
    fun toolbarActionsAreVisibleAndClickableWhenThereAreNoSuggestions() {
        ...
        compose.onNodeWithContentDescription("Typing guide").assertDoesNotExist()
    ...
    fun suggestionTapCallsTheProvidedHandler() {
        ...
        compose.setContent {
            TestKeyboardHost {
                SuggestionArea(
                    ...
                    onClipboard = {},
                    onEmoji = {},
                    onDismissSuggestions = { dismissed = true }
                )
            }
        }
        ...
        compose.onNodeWithContentDescription("Dismiss suggestions").performClick()
    ...
    fun completionToPendingToPredictionNeverRendersToolbarOrStaleChips() {
        ...
        compose.onNodeWithContentDescription("Settings").assertDoesNotExist()
        compose.onNodeWithTag(SuggestionAreaTestContract.PREDICTION_LOADING_STRIP_TAG)
            .assertIsDisplayed()
        repeat(3) { index ->
            compose.onNodeWithTag(SuggestionAreaTestContract.predictionLoadingSlotTag(index))
                .assertExists()
    ...
    fun predictionLoadingIndicatorIsDelayedForOneHundredTwentyMilliseconds() {
        ...
        compose.onNodeWithTag(SuggestionAreaTestContract.PREDICTION_LOADING_STRIP_TAG)
            .assertExists()
        compose.onNodeWithTag(SuggestionAreaTestContract.PREDICTION_LOADING_INDICATOR_TAG)
            .assertDoesNotExist()
        compose.mainClock.advanceTimeBy(
            SuggestionAreaTestContract.PREDICTION_LOADING_INDICATOR_DELAY_MILLIS - 1L,
    ...
    fun languageLoadingAndPrivateStatesDoNotFallThroughToToolbar() {
        ...
        compose.onNodeWithTag(SuggestionAreaTestContract.LANGUAGE_LOADING_INDICATOR_TAG)
            .assertIsDisplayed()
        compose.onNodeWithContentDescription("Settings").assertDoesNotExist()
    ...
    fun twoAndThreeAmharicSuggestionsUseThreeEqualCenteredSlots() {
        ...
        for (count in 2..3) {
            ...
            val stripBounds = compose
                .onNodeWithTag(SuggestionAreaTestContract.AMHARIC_SUGGESTION_STRIP_TAG)
                .fetchSemanticsNode()
                .boundsInRoot
    ...
    @Test
    fun oneFourAndFiveAmharicSuggestionsUseThreeVisibleSlots() {
        ...
        for (count in listOf(1, 4, 5)) {
            ...
            val stripBounds = compose
                ...
                .fetchSemanticsNode()
                .boundsInRoot
            visibleWords.take(SUGGESTION_STRIP_VISIBLE_LIMIT)
                .forEachIndexed { index, word ->
                    val wordCenter = compose
                        ...
                        .x
                    val expectedCenter =
                        stripBounds.left + stripBounds.width * (index + 0.5f) /
                            SUGGESTION_STRIP_VISIBLE_LIMIT

                    assertTrue(
                        "$count Amharic suggestions should use equal visible slots",
                        kotlin.math.abs(wordCenter - expectedCenter) <= 2f
                    )
                }
            visibleWords.drop(SUGGESTION_STRIP_VISIBLE_LIMIT).forEach { word ->
    ...
    @Test
    fun overflowingAmharicSuggestionsUseTheExpandedView() {
        ...
        words.take(SUGGESTION_STRIP_VISIBLE_LIMIT).forEach { word ->
            compose.onNodeWithText(word).assertIsDisplayed()
        }
        words.drop(SUGGESTION_STRIP_VISIBLE_LIMIT).forEach { word ->
            compose.onNodeWithText(word).assertDoesNotExist()
        }
        compose.onNodeWithContentDescription("Show more suggestions").performClick()
        compose.runOnIdle { assertTrue(expandedRequested) }
