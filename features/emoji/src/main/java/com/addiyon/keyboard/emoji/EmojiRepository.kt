...
class EmojiRepository(
    ...
    fun loadAsync(onReady: () -> Unit = {}) {
        ...
        worker = Thread {
            try {
                ...
                logFailure(oom, "setThreadPriority OOM")
            } catch (t: Throwable) {
                logFailure(t, "setThreadPriority")
            }
            val loaded = try {
                load(loadGeneration)
            } catch (oom: OutOfMemoryError) {
                logFailure(oom, "load OOM")
                onOutOfMemory()
                null
            } catch (t: Throwable) {
                logFailure(t, "load")
            ...
            mainHandler.post {
                try {
                    ...
                    if (loadGeneration != generation) return@post
                    data = loaded
                    val readyCallbacks = callbacks.toList()
                    callbacks.clear()
                    readyCallbacks.forEach { it() }
                } catch (t: Throwable) {
                    logFailure(t, "publish")
                }
            }
        }
    ...
    private fun logFailure(throwable: Throwable, operation: String) {
        Log.w("AddiyonEmoji", operation, throwable)
    }
}
