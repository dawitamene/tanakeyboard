...
class NgramContextTest {
    ...
    private val amharic = NgramContext(
        isWordChar = {
            it in 'ሀ'..'ፚ' || it in '፝'..'፟' ||
                it in 'ᎀ'..'ᎏ' || it in 'ⶀ'..'ⷞ' || it in 'ꬁ'..'ꬮ'
        },
        normalizeWord = {
            Regex("[፝-፟]").replace(Normalizer.normalize(it, Normalizer.Form.NFC), "")
        }
    )
    private val english = NgramContext(
        isWordChar = { it.isLetter() || it == '\'' || it == '’' },
        normalizeWord = { it.replace('’', '\'') }
    )

    private fun ctx(text: String?) = amharic.extract(text)
    private fun en(text: String?) = english.extract(text)
