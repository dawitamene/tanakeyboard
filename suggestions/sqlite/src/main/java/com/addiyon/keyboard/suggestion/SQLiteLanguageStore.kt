...
class SQLiteLanguageStore(
    context: Context,
    val assetName: String,
    private val metadataAssetName: String,
    ...
    fun loadAsync(onComplete: () -> Unit) {
        ...
        ioExecutor.execute {
            ...
            val opened = try {
                ...
                onFailure(oom, "SQLiteLanguageStore load OOM")
                onOutOfMemory()
                null
            } catch (t: Throwable) {
                onFailure(t, "SQLiteLanguageStore load")
                null
            }
            postToMain main@{
                ...
                notify.forEach { callback ->
                    try {
                        ...
                        onFailure(t, "SQLiteLanguageStore callback")
                    }
                }
            }
        }
    ...
    fun release() {
        ...
        try {
            ...
            onFailure(t, "SQLiteLanguageStore release")
        }
    }
    ...
    fun handleQueryFailure(t: Throwable) {
        ...
        try {
            ...
            onFailure(closeFailure, "SQLiteLanguageStore failure close")
        }
    }

    fun reportFailure(t: Throwable, operation: String) {
        onFailure(t, operation)
    }
    ...
    private fun postToMain(block: () -> Unit) {
        ...
        try {
            ...
            onFailure(t, "SQLiteLanguageStore post")
            runnable.run()
        }
    }
    ...
    private fun openReadOnly(file: File): SQLiteDatabase =
        SQLiteDatabase.openDatabase(
            ...
            DatabaseErrorHandler { corrupt ->
                onWarning("Corrupt dictionary database ${corrupt.path}")
            },
        )
