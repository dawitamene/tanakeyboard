...
class KeyboardMetricsTest {
    ...
    fun heightIsCappedAtTheLowerBoundOnNarrowScreens() {
        val metrics = computeKeyboardMetrics(StandardNumberLayout.rows, 240.dp)
    ...
    fun keypadColumnsOverrideTheCharacterCountHeuristic() {
        val metrics = computeKeyboardMetrics(
            StandardKeypadLayout.rows,
            420.dp,
            columns = StandardKeypadLayout.columns
        )
    ...
    fun keypadFixedRowsFillItsDeclaredColumnsExactly() {
        val columns = StandardKeypadLayout.columns!!
    ...
    fun keypadUsesTheSameKeyHeightAsStandardLayouts() {
        val keypad = computeKeyboardMetrics(
            StandardKeypadLayout.rows,
            320.dp,
            columns = StandardKeypadLayout.columns
        )
        val symbols = computeKeyboardMetrics(StandardSymbolsLayout.rows, 320.dp)
    ...
    fun everyControlRowFillsTheAvailableWidth() {
        for (row in listOf(
            ...
            StandardNumberLayout.rows[2],
            StandardSymbolsLayout.rows[2],
            StandardMoreSymbolsLayout.rows[2],
