...
fun CharacterKey(
    ...
    KeyButton(
        ...
        secondaryFontSize = presentation.secondaryFontSize,
        ...
        onLongPress = presentation.longPressCommit?.let { value ->
            { onCommitText(value) }
        },
...
data class CharacterKeyPresentation(
    val primaryText: String,
    val secondaryText: String? = null,
    val longPressCommit: String? = null,
    val secondaryFontSize: TextUnit = 10.sp
)

fun KeyData.Character.usesSpecialBackground(): Boolean =
...
fun RowScope.NumberToggleKey(
    ...
    KeyButton(
        primaryText = when {
            isNumberMode -> "ABC"
            alternateNumbersLabel != null -> alternateNumbersLabel
            else -> "?123"
        },
...
fun RowScope.SymbolsToggleKey(
    ...
    KeyButton(
        primaryText = when {
            ...
            numbersMode == NumbersMode.NUMBERS && alternateNumbersLabel != null -> alternateNumbersLabel
...
/**
 * Language toggle showing the active pack over the pack a tap will activate.
 * The presentation rotates with the active pack, so products with three or
 * more languages never hide labels that do not fit in a fixed two-line key.
 */
@Composable
fun RowScope.LanguageToggleKey(
    presentation: LanguageKeyPresentation,
    height: Dp,
    vibrateOnKeypress: Boolean,
    soundOnKeypress: Boolean,
    onClick: () -> Unit
    ...
    KeyButton(
        ...
        content = {
            if (presentation.switchesInputMethod) {
                Icon(
...
data class LanguageKeyPresentation(
    val topLabel: String,
    val bottomLabel: String,
    val switchesInputMethod: Boolean = false
) {
    companion object {
        fun systemSwitcher(): LanguageKeyPresentation = LanguageKeyPresentation(
            topLabel = "",
            bottomLabel = "",
            switchesInputMethod = true
        )

        fun cycling(labels: List<String>, activeIndex: Int): LanguageKeyPresentation {
            require(labels.isNotEmpty())
            require(activeIndex in labels.indices)
            return LanguageKeyPresentation(
                topLabel = labels[activeIndex],
                bottomLabel = labels[(activeIndex + 1).mod(labels.size)]
            )
        }
    }
}
