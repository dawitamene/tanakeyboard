...
class TypingController(
    private val editor: EditorOperations,
    ...
    fun onSuggestionTap(
        ...
        if (kind == SuggestionKind.COMPLETION && !composition.isActive) {
            val current = profile()
            replaceCommittedDisplayWord(current, word, trailingSpace)?.let { replaced ->
                onCompositionChanged()
                return@applyingOwnEdit replaced
            }
            adoptWordAtCaret(current)
        }
    ...
    private fun replaceCommittedDisplayWord(
        current: TypingProfile,
        replacement: String,
        trailingSpace: Boolean
    ): Boolean? {
        if (!current.remembersRawLatin || !current.allowsAdoption) return null
        val context = WordAdoption.inspect(
            editor = editor,
            isWordCharacter = { char -> current.isWordCharacter(char.toString()) },
            wordEndingAtCursor = current.wordEndingAtCursor
        ) as? WordAdoption.CaretContext.AtWordEnd ?: return null
        if (resumedWords.rawLatinFor(context.word) != null) return null
        if (!WordAdoption.adopt(editor, context.word)) return false
        val display = if (trailingSpace) "$replacement " else replacement
        val accepted = editor.commitText(display)
        if (accepted) onWordCommitted(replacement)
        return accepted
    }
