 ...
 * so both product settings UIs and the shared keyboard runtime
...
object KeyboardPrefs {
    ...
    fun prefs(context: Context): SharedPreferences {
        val current = context.getSharedPreferences(FILE, Context.MODE_PRIVATE)
        migrateLegacyProductPreferences(context, current)
        return current
    }
    ...
    private fun migrateLegacyProductPreferences(
        context: Context,
        current: SharedPreferences
    ) {
        if (current.all[KEY_LEGACY_PRODUCT_MIGRATED] == true) return
        try {
            val legacy = context.getSharedPreferences(
                LEGACY_PRODUCT_FILE,
                Context.MODE_PRIVATE
            )
            val values = LegacyKeyboardPreferenceMigration.valuesToCopy(
                currentKeys = current.all.keys,
                legacyValues = legacy.all
            )
            current.edit().apply {
                values.forEach { (key, value) -> putPreferenceValue(key, value) }
                putBoolean(KEY_LEGACY_PRODUCT_MIGRATED, true)
            }.apply()
        } catch (_: Throwable) {
        }
    }
    ...
    private const val LEGACY_PRODUCT_FILE = "keyboard_product_runtime"
    private const val KEY_LEGACY_PRODUCT_MIGRATED = "legacy_product_preferences_migrated"
}

internal object LegacyKeyboardPreferenceMigration {
    private val supportedKeys = setOf(
        KeyboardPrefs.KEY_VIBRATE,
        KeyboardPrefs.KEY_SOUND,
        KeyboardPrefs.KEY_PALETTE,
        KeyboardPrefs.KEY_NUMBER_ROW,
        KeyboardPrefs.KEY_KEYBOARD_HEIGHT_SCALE,
        KeyboardPrefs.KEY_AMHARIC_MODE,
        KeyboardPrefs.KEY_ACTIVE_LANGUAGE_ID,
        KeyboardPrefs.KEY_RECENT_EMOJIS,
        KeyboardPrefs.KEY_EMOJI_SKIN_TONES,
        KeyboardPrefs.KEY_FEATURE_TOUR_SEEN,
        KeyboardPrefs.KEY_USAGE_SESSIONS,
        KeyboardPrefs.KEY_REVIEW_PROMPTED,
        KeyboardPrefs.KEY_PERSONAL_DICTIONARY
    )

    fun valuesToCopy(
        currentKeys: Set<String>,
        legacyValues: Map<String, *>
    ): Map<String, Any> = legacyValues.mapNotNull { (key, value) ->
        if (key !in supportedKeys || key in currentKeys || value == null) null
        else key to value
    }.toMap()
}
